...
const explicitUrl = getArg('--url') || process.env.EVENT_INGEST_URL;
...
if (!secret) {
  console.error('Missing CRON_SECRET or EVENT_INGEST_SECRET. Add it to your environment or .env.local.');
  process.exit(1);
}

const url = new URL('/api/jobs/ingest-events', baseUrl);

console.log(`Running event ingestion for ${normalizedTarget}: ${url.origin}`);
...
if (!response.ok) {
  console.error(`Event ingestion failed: ${response.status} ${response.statusText}`);
